...
public class map<Key extends Comparable<Key>, Value> {
    ...
    private Node balance(Node x) {
        if (balanceFactor(x) < -1) {
            if (balanceFactor(x.right) > 2) {
                x.right = rotateRight(x.right);
            }
            x = rotateLeft(x);
        }
        else if (balanceFactor(x) > 1) {
            if (balanceFactor(x.left) < -2) {
    ...
    private void printBalance(Node x){
        if(x != null){
            printBalance(x.left);
            System.out.printf("%s ", balanceFactor(x));
            printBalance(x.right);
        }
    }
    ...
    public Value get(Key key) {
        ...
        System.out.printf("Ваше искомое значение: ");
        return x.val;
    }
